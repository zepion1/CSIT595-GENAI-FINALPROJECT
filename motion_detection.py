...
MOTION_THRESHOLD = 8000                        # Min changed pixels to trigger
BLUR_KERNEL = (21, 21)                         # Gaussian blur kernel size
MIN_CONTOUR_AREA = 3000                        # Ignore tiny contours (noise)
COOLDOWN_SECONDS = 10                          # Seconds between webhook sends
JPEG_QUALITY = 85                              # Image quality sent to server (1-100)
SUSTAINED_FRAMES = 4                           # Consecutive frames with motion required
                                               # before a webhook fires. Raise this to
...
def build_preview(
    ...
    # --- Legend (bottom-right) --------------------------------------------
    legend_lines = ["q — quit", "p — pause"]
    for i, line in enumerate(reversed(legend_lines)):
        cv2.putText(
            canvas, line,
            (canvas.shape[1] - 120, canvas.shape[0] - 10 - i * 18),
...
def run():
    ...
    log.info("Motion detection started (threshold=%d px², sustained=%d frames)",
    ...
    try:
        while True:
            ...
            if not paused:
                if motion:
                    sustained_count += 1
                    log.debug("Sustained motion count: %d/%d",
                              sustained_count, SUSTAINED_FRAMES)
                else:
                    # Reset as soon as a quiet frame appears
                    sustained_count = 0

                # Only fire the webhook once motion has held for enough frames
                if sustained_count >= SUSTAINED_FRAMES and cooldown_remaining == 0:
                    ...
                    if send_webhook(frame, contours):
                        last_sent = now
                    sustained_count = 0   # Reset so it must re-confirm next time
